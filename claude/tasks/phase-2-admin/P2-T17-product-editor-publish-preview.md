...
# Notes for next agent
  ...
  - **PDP consumer is Phase 3.** The preview link generated by
    `generatePreviewLinkAction` points at `/p/<slug>?preview=...`
    which doesn't exist yet. The storefront PDP must, when it
    lands, call `verifyPreviewToken(token, productId)` and bypass
    `is_published` if the call returns non-null. The token signs
    the product id (subject) so a leaked token only previews the
    one product it was issued for.

  - **`PREVIEW_TOKEN_SECRET`** added to `.env.example` and the
    local `.env.local` (generated via `openssl rand -base64 48`).
    Add to the Vercel project's env vars before the storefront PDP
    ships, otherwise preview links 500.

  - **Pre-flight checks live in `lib/db/admin/publish.ts`.** Seven
    checks today (4 blocking, 3 warning). Adding a check is a
    matter of pushing a `PreflightCheck` onto the array — keep the
    `id` stable since the audit log + failed-publish error payload
    reference it.

  - **Five categories of revalidation on publish.** The action
    revalidates: `updateTag('products')`, the PDP path, the
    category page (if categorized), and `/` (if `is_featured`).
    Featured products are rare enough that the extra revalidatePath
    isn't measurable overhead.

  - **DB trigger is load-bearing.** Manually issuing
    `UPDATE products SET review_status='published' WHERE
    is_published=false` is rejected by
    `products_publish_state_consistency` from 0004. The integration
    test confirms; don't disable the trigger if it ever blocks a
    legitimate write — fix the writer instead.

  - **No scheduled-publish.** The task spec called it out as
    deferred; the Publish tab UI just doesn't render that option.

  - **9-way parallel page fetch** now. Was 7-way after T16 added
    images. Page-load time should be roughly the same since each
